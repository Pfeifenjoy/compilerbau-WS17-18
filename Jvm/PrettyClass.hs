...
Wed May 26 10:24:58 BOT 2010
    Added support for program counter in the attribute code
   , now all code is well enumerated.
-}
...
instruction2pp (inst:xs) counter = 
    case inst of
        ...
        16  -> (pp counter >#< text "bipush" >#< pp (take 1 xs))
               :instruction2pp (drop 1 xs) (counter + 2)
        17  -> (pp counter >#< text "sipush" >#< let [b1,b2] = take 2 xs 
                                                 in pp b1 >#< pp b2)
                :instruction2pp (drop 2 xs) (counter + 3)
...
{-
fromBytes2Tupla xs = (\(_, lst1, lst2) -> zip lst1 lst2) 
                       $ foldr cons nil (toInts xs)
    where nil = (0, [], [])
          cons b (v, lst1, lst2) = if odd v 
                                   then (v+1, lst1, b:lst2) 
                                   else (v+1, b:lst1, lst2)
-}
